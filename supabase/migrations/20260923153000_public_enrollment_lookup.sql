...
begin
  ...
  select jsonb_build_object('found', true, 'status', a.data->>'status', 'submittedAt', (a.data->>'submission_date')::numeric, 'reviewedAt', (a.data->>'reviewed_at')::numeric, 'rejectionReason', nullif(a.data->>'rejection_reason', ''), 'clarificationFields', coalesce(a.data->'clarification_fields', '[]'::jsonb)) into result
  ...
  select jsonb_build_object('found', true, 'status', 'approved') into result from public.rmc_profiles p where trim(p.profile->>'student_id') = trim(lookup_student_id) and p.status = 'active' limit 1;
...
returns jsonb language sql security invoker set search_path = '' as $$ select app_private.lookup_enrollment(lookup_student_id) $$;
