-- An authenticated applicant may correct and resubmit the same application
-- without entering the section key again. A key remains required if the
-- applicant changes their academic assignment during resubmission.
...
begin
  ...
  if person is not null then
    ...
    if n is null or trim(n) = '' then
      raise exception 'Choose a valid academic assignment.';
    end if;
    if app_private.frozen(n) then
      raise exception 'Enrollment is frozen.';
    end if;

    -- Reusing the assignment already attached to this authenticated
    -- application does not require the student to enter the key again.
    if n is distinct from p.node_id then
      ...
      if secret is not null
        ...
        raise exception 'Enter the enrollment key for the selected section.';
      end if;
    end if;
