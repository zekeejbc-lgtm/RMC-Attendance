...
begin
 ...
 when action in ('updateContactDetails','updatePhoto') then
  ...
  if action='updatePhoto' then
   if d->>'path' like 'storage://' || p.id::text || '/%' then
    if not exists(select 1 from storage.objects where bucket_id='rmc-documents' and name=substring(d->>'path' from 11) and metadata->>'mimetype' in ('image/jpeg','image/png','image/webp')) then raise exception 'Upload a valid profile photo first.'; end if;
   elsif coalesce(d->>'path','') !~ '^https://drive[.]google[.]com/(uc[?]export=view[&]id=|thumbnail[?]id=)[A-Za-z0-9_-]+([&]sz=[A-Za-z0-9_-]+)?$' then
    raise exception 'Use a valid public Google Drive profile photo URL.';
   end if;
