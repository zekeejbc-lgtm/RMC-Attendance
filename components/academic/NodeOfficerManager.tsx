...
export function NodeOfficerManager({ node, actor, canManage, onChanged }: Props) {
  ...
  const officers = typeof appData.getOfficialsForNode === 'function' ? appData.getOfficialsForNode(node.id) : [];
  const eligibleExisting = useMemo(() => (typeof appData.getOfficialAccounts === 'function' ? [...appData.getOfficialAccounts(), ...(['admin', 'ossa'].includes(actor.role) ? appData.getVisibleStudents(actor.uid).filter(account => account.role === 'student') : [])] : []).filter((account) => (allowedRoles.includes(account.role) || account.role === 'student') && account.uid !== actor.uid && (actor.role === 'admin' || !account.official_data?.assignment_node_id || appData.isNodeVisibleTo(actor.uid, account.official_data.assignment_node_id)) && !officers.some((officer) => officer.uid === account.uid)), [node.id, officers.length]);
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800">
      ...
      {isSectionNode && <div className="mt-4 space-y-3 rounded-xl border border-amber-200 bg-amber-50/70 p-4 dark:border-amber-900 dark:bg-amber-950/30">
        <h4 className="text-sm font-bold">Section enrollment key</h4>
        <p className="text-xs text-slate-600 dark:text-slate-300">{node.enrollmentKeyRequired ? 'Enrollment requires a key. Existing keys cannot be retrieved; set a new key if needed.' : 'Set a key to require it when students apply to this section.'}</p>
